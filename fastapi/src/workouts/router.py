...
from .schemas import WorkoutBaseSchema, WorkoutWithExercisesBaseSchema, WorkoutLogBaseSchema
...
def fetch_workout_logs(
    ...
    w = get_workout_logs(user_id=current_user.id, workout_id=workout_id, session=session)
    if not w:
