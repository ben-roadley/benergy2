...
from ..catalog.models import CatalogExercisedefinition
...
class WorkoutWorkout(SQLModel, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            deferrable=True,
            initially="DEFERRED",
            name="workout_workout_user_id_9f8fbd53_fk_auth_user_id",
        ),
    ...
    updated_at: datetime.datetime = Field(
        sa_column=Column("updated_at", DateTime(True), nullable=False)
    )
    ...
    workout_warmupsuggestion: "WorkoutWarmupsuggestion" = Relationship(
        back_populates="workout", sa_relationship_kwargs={"uselist": False}
    )
    workout_workoutlog: list["WorkoutWorkoutlog"] = Relationship(
...
class WorkoutExercise(SQLModel, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["catalog_exercisedefinition.slug"],
            deferrable=True,
            initially="DEFERRED",
            name="workout_exercise_exercise_definition__0d289ed6_fk_catalog_e",
        ),
        ForeignKeyConstraint(
            ["workout_id"],
            ["workout_workout.id"],
            deferrable=True,
            initially="DEFERRED",
            name="workout_exercise_workout_id_281489e7_fk_workout_workout_id",
        ),
        PrimaryKeyConstraint("id", name="workout_exercise_pkey"),
        UniqueConstraint("workout_id", "order", name="unique_order_for_workout"),
        Index(
            "workout_exercise_exercise_definition_id_0d289ed6", "exercise_definition_id"
        ),
        Index(
            "workout_exercise_exercise_definition_id_0d289ed6_like",
            "exercise_definition_id",
            postgresql_ops={"exercise_definition_id": "varchar_pattern_ops"},
        ),
    ...
    exercise_definition_id: str = Field(
        sa_column=Column("exercise_definition_id", String(100), nullable=False)
    )
    rest_time_after: int = Field(
        sa_column=Column("rest_time_after", SmallInteger, nullable=False)
    )
...
class WorkoutSetofreps(SQLModel, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            deferrable=True,
            initially="DEFERRED",
            name="workout_setofreps_exercise_id_3a05f215_fk_workout_exercise_id",
        ),
    ...
    exercise_id: int = Field(
        sa_column=Column("exercise_id", BigInteger, nullable=False)
    )
...
class WorkoutWarmupsuggestion(SQLModel, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            deferrable=True,
            initially="DEFERRED",
            name="workout_warmupsugges_workout_id_5f8efa9f_fk_workout_w",
        ),
    ...
    exercises_hash: str = Field(
        sa_column=Column("exercises_hash", String(64), nullable=False)
    )
    suggestions: dict = Field(sa_column=Column("suggestions", JSONB, nullable=False))
    generated_at: datetime.datetime = Field(
        sa_column=Column("generated_at", DateTime(True), nullable=False)
    )
    workout_id: int = Field(sa_column=Column("workout_id", BigInteger, nullable=False))
...
class WorkoutWorkoutlog(SQLModel, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            deferrable=True,
            initially="DEFERRED",
            name="workout_workoutlog_user_id_d6499a67_fk_auth_user_id",
        ),
        ForeignKeyConstraint(
            ["workout_id"],
            ["workout_workout.id"],
            deferrable=True,
            initially="DEFERRED",
            name="workout_workoutlog_workout_id_f7a63ded_fk_workout_workout_id",
        ),
    ...
    completed_at: datetime.datetime = Field(
        sa_column=Column("completed_at", DateTime(True), nullable=False)
    )
    user_id: int = Field(sa_column=Column("user_id", Integer, nullable=False))
    workout_id: int = Field(sa_column=Column("workout_id", BigInteger, nullable=False))
...
class WorkoutWorkoutlogentry(SQLModel, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["log_id"],
            ["workout_workoutlog.id"],
            deferrable=True,
            initially="DEFERRED",
            name="workout_workoutlogen_log_id_b4981df9_fk_workout_w",
        ),
        ForeignKeyConstraint(
            ["set_of_reps_id"],
            ["workout_setofreps.id"],
            deferrable=True,
            initially="DEFERRED",
            name="workout_workoutlogen_set_of_reps_id_a1e6f826_fk_workout_s",
        ),
    ...
    nb_reps_target: int = Field(
        sa_column=Column("nb_reps_target", SmallInteger, nullable=False)
    )
    nb_reps_actual: int = Field(
        sa_column=Column("nb_reps_actual", SmallInteger, nullable=False)
    )
    log_id: int = Field(sa_column=Column("log_id", BigInteger, nullable=False))
    set_of_reps_id: int = Field(
        sa_column=Column("set_of_reps_id", BigInteger, nullable=False)
    )
    weight_actual: Optional[decimal.Decimal] = Field(
        default=None, sa_column=Column("weight_actual", Numeric(6, 2))
    )
    weight_target: Optional[decimal.Decimal] = Field(
        default=None, sa_column=Column("weight_target", Numeric(6, 2))
    )

    log: "WorkoutWorkoutlog" = Relationship(back_populates="workout_workoutlogentry")
    set_of_reps: "WorkoutSetofreps" = Relationship(
        back_populates="workout_workoutlogentry"
    )
