...
class AuthUser(SQLModel, table=True):
    ...
    is_superuser: bool = Field(
        sa_column=Column("is_superuser", Boolean, nullable=False)
    )
    ...
    date_joined: datetime.datetime = Field(
        sa_column=Column("date_joined", DateTime(True), nullable=False)
    )
    last_login: Optional[datetime.datetime] = Field(
        default=None, sa_column=Column("last_login", DateTime(True))
    )

    users_userprofile: "UsersUserprofile" = Relationship(
...
class UsersUserprofile(SQLModel, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            deferrable=True,
            initially="DEFERRED",
            name="users_userprofile_user_id_87251ef1_fk_auth_user_id",
        ),
    ...
    display_name: str = Field(
        sa_column=Column("display_name", String(100), nullable=False), default=""
    )
    sex: SexChoices = Field(sa_column=Column("sex", String(20), nullable=False))
    fitness_level: FitnessLevelChoices = Field(
        sa_column=Column("fitness_level", String(20), nullable=False)
    )
    ...
    session_duration: SessionDurationChoices = Field(
        sa_column=Column("session_duration", String(10), nullable=False)
    )
    injury_history: str = Field(
        sa_column=Column("injury_history", Text, nullable=False)
    )
    lifestyle_description: str = Field(
        sa_column=Column("lifestyle_description", Text, nullable=False)
    )
    sleep_quality: SleepQualityChoices = Field(
        sa_column=Column("sleep_quality", String(10), nullable=False)
    )
    stress_level: StressLevelChoices = Field(
        sa_column=Column("stress_level", String(10), nullable=False)
    )
    user_id: int = Field(sa_column=Column("user_id", Integer, nullable=False))
    date_of_birth: Optional[datetime.date] = Field(
        default=None, sa_column=Column("date_of_birth", Date)
    )
    weight_kg: Optional[decimal.Decimal] = Field(
        default=None, sa_column=Column("weight_kg", Numeric(5, 1))
    )
    height_cm: Optional[int] = Field(
        default=None, sa_column=Column("height_cm", SmallInteger)
    )
    training_days_per_week: Optional[int] = Field(
        default=None, sa_column=Column("training_days_per_week", SmallInteger)
    )
